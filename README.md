# [Nintendo 64DD](https://en.wikipedia.org/wiki/64DD) for [MiSTer Platform](https://github.com/MiSTer-devel/Main_MiSTer/wiki)

This core extends the [MiSTer Nintendo 64](https://github.com/MiSTer-devel/N64_MiSTer/) implementation with Nintendo 64DD drive emulation.
...
Place the files in the N64DD game directory:
    ...
    /media/fat/games/N64DD/boot3.rom => Japan Retail Nintendo 64DD IPL ROM
...
The cartridge can also be loaded automatically by giving it the same base name as the NDD image with a `.rom` extension:
    ...
    /media/fat/games/N64DD/Game Name/Game Name.ndd
    /media/fat/games/N64DD/Game Name/Game Name.rom
...
64DD writes are stored separately from the original NDD image. The preferred save is the writable RAM area of the disk, stored under the core save directory with the same base name as the NDD image:
    ...
    /media/fat/saves/N64DD/Game Name.ram
